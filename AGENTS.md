...
- `templates/` Tera templates; `partials/` holds head, nav, footer, the logo (`logo.html`), the GitHub icon (`icon-github.html`), the borrowing figure (`home-figure.html`), and the hero figure (`home-service.html`); `robots.txt` allows all crawlers and declares Content Signals.
...
- `scripts/agents.sh` writes a Markdown copy of each page (the home page copy starts with the hero lead from `extra.lead`) and `/llms.txt`, and checks the borrowing figure against the borrowing example.
...
- **Copies must stay exact.** `content/learn/reference.md` is `docs/language.md` from the compiler repository, with the source commit noted at the top; the examples and the generated Rust on the home and examples pages match `examples/` and the compiler README byte for byte. Refresh them by copying, not by editing. The borrowing figure (`templates/partials/home-figure.html`) is a marked-up copy of the borrowing example in `content/learn/examples.md`; the build fails if the two differ, so update both together. The hero figure (`templates/partials/home-service.html`) is the opposite case: a design target the compiler does not accept yet, labelled as coming in its milestone. It stays in the template and never goes into `content/`, so the Markdown copies do not teach agents syntax that does not compile; replace it with a real example from `examples/` once one exists.
